...
object ModEntityGeneration {
    ...
    private fun addEntityToAllBiomesExceptThese(
        ...
        val isBiomeSelected: Boolean =
            biomes.map(ResourceKey<*>::location).map(Any::toString).any { it == event.name.toString() }
        if (!isBiomeSelected) {
            addEntityToAllBiomes(event, type, weight, minCount, maxCount)
        }
    }

    private fun addEntityToSpecificBiomes(
        event: BiomeLoadingEvent,
        type: EntityType<*>,
        weight: Int,
        minCount: Int,
